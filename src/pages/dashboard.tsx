...
import { getBookingHistories } from 'apiSdk/booking-histories';
import { BookingHistoryInterface } from 'interfaces/booking-history';
import { getCarStatuses } from 'apiSdk/car-statuses';
import { CarStatusInterface } from 'interfaces/car-status';
import { getOperationStaffs } from 'apiSdk/operation-staffs';
import { OperationStaffInterface } from 'interfaces/operation-staff';

import { UserLocationListPage as ShowCaseList } from 'pages/user-locations';

function DashboardPage() {
  const { session } = useSession();

  const bookingHistoryStat = useSWR<PaginatedInterface<BookingHistoryInterface>>(
    () => `/booking-histories`,
    () => getBookingHistories({ limit: 1, offset: 0 }),
  );

  const carStatusStat = useSWR<PaginatedInterface<CarStatusInterface>>(
    () => `/car-statuses`,
    () => getCarStatuses({ limit: 1, offset: 0 }),
  );

  const operationStaffStat = useSWR<PaginatedInterface<OperationStaffInterface>>(
    () => `/operation-staffs`,
    () => getOperationStaffs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Booking Histories',
      result: bookingHistoryStat?.data?.totalCount,
      isLoading: bookingHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Car Statuses',
      result: carStatusStat?.data?.totalCount,
      isLoading: carStatusStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Operation Staffs',
      result: operationStaffStat?.data?.totalCount,
      isLoading: operationStaffStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`user-locations`} passHref legacyBehavior>
                  <Button
