...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getUserLocationsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'user_location');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.user_location.count(countOptions as unknown),
  ...
  async function getUserLocations() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.user_location
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'user_location'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createUserLocation() {
    await userLocationValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.user_location.create({
      data: body,
    });
