...
import { getCarStatusById } from 'apiSdk/car-statuses';
import { CarStatusInterface } from 'interfaces/car-status';

function CarStatusViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<CarStatusInterface>(
    () => (id ? `/car-statuses/${id}` : null),
    () =>
      getCarStatusById(id, {
        relations: [],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('car_status', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/car-statuses/edit/${id}`} passHref legacyBehavior>
                    <Button
